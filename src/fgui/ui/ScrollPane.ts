...
var s_endPos: Vec2 = new Vec2();
var s_oldChange: Vec2 = new Vec2();
...
export class ScrollPane {
    ...
    private _maskContainer: UIElement;
    ...
    public setup(buffer: ByteBuffer): void {
        ...
        if ((flags & 512) == 0) this._owner.element.clipping = true; //dont clip flag
        if ((flags & 1024) != 0) this._floating = true;
        if ((flags & 2048) == 0) this._maskContainer.clipping = true; //dont clip margin flag
    ...
    private __touchMove(owner : GObject): void {
        ...
        */
        var deltaGlobalPositionX: number = this._lastTouchGlobalPos.x - cpos[0];
        var deltaGlobalPositionY: number = this._lastTouchGlobalPos.y - cpos[1];
        if (deltaPositionX != 0)
    ...
    private alignPosition(pos: Vec2, inertialScrolling: boolean): void {
        if (this._pageMode) {
            ...
            var pt: Vec2 = this._owner.getSnappingPositionWithDir(-pos.x, -pos.y, xDir, yDir, s_vec2);
    ...
    private updateTargetAndDuration2(pos: number, axis: AxisType): number {
        ...
        if (pos > 0)
            ...
            if (touchScreen)
                v2 *= 1136 / Math.max(Game.GetScreenWidth(), Game.GetScreenHeight());
